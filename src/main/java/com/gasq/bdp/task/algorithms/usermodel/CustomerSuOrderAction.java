...
public class CustomerSuOrderAction {
...
//		            	String request_date = vs[0];
    ...
    public static void main(String[] args) throws Exception {
    	System.out.println("CustomerSuOrderAction传入参数：参数1=："+args[0]+"\t"+args[1]+"\t"+args[2]);
    	Instant start = Instant.now();
    	Configuration conf = new Configuration();
    	conf.set("alph", args[2]);
        Job job = Job.getInstance(conf,"CustomerSuOrderAction");
        job.setNumReduceTasks(1);
        job.setJarByClass(CustomerSuOrderAction.class);
...
//        alph = 1;
        ...
        System.out.println("CustomerSuOrderAction计算用时："+Duration.between(start, end)+"ms！");
        System.exit(f ? 0 : 1);
    }
}