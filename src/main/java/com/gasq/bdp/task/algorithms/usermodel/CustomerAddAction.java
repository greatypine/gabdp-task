...
//		            	String request_date = vs[0];
    ...
    public static void main(String[] args) throws Exception {
    	System.out.println("CustomerAddAction传入参数：参数1=："+args[0]+"\t"+args[1]+"\t"+args[2]);
    	Instant start = Instant.now();
    	Configuration conf = new Configuration();
    	conf.set("alph", args[2]);
        Job job = Job.getInstance(conf,"CustomerAddAction");
        job.setNumReduceTasks(1);
        job.setJarByClass(CustomerAddAction.class);
...
//        alph = 0.5;
        ...
        System.out.println("CustomerAddAction计算用时："+Duration.between(start, end).toMillis()+"ms！");
        System.exit(f ? 0 : 1);
    }
}